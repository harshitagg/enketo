...
class Webform extends CI_Controller {
	...
	public function post_instance()
	{
		...
		if (!isset($subdomain))
		{
			show_error('This controller is only available when a subdomain is used');
		}
		else
		{
			extract($_POST);
			if (!empty($instance) && !empty($return_url))
			{
				...
				$this->session->set_userdata($subdomain, $instance);
				$this->session->set_userdata($subdomain.'_return', $return_url);

				//$session_instance = $this->session->userdata($result['subdomain']);
				//echo 'instance'.$session_instance;
	...
	public function edit_session()
	{
		...
		$db_subd = substr($subdomain, 0, strlen($subdomain)-2);
		$instance = $this->session->userdata($db_subd);
		$return_url = $this->session->userdata($db_subd.'_return');
 		$this->session->unset_userdata($db_subd);
 		$this->session->unset_userdata($db_subd.'_return');
		/*********************************************/
